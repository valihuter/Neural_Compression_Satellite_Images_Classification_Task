...
def compress_image(input_path, output_dir, target_bpp_x100):
    ...
    try:
        ...
        # Encode with JPEG-AI
        # IMPORTANT: CUDA_VISIBLE_DEVICES must be set for GPU to work!
        import os
...
def get_all_images(class_filter=None):
    ...
    for class_dir in sorted(INPUT_ROOT.iterdir()):
        ...
        if class_filter and class_dir.name not in class_filter:
            continue
...
def main():
    ...
    parser.add_argument('--classes', type=str, default='all',
                       help='Classes to process: "all" or comma-separated like "airplane,airport,beach"')
    ...
    if args.classes != 'all':
        ...
        print(f"Classes filter: {len(class_filter)} classes")
    
    # Find all images
    test_images = get_all_images(class_filter)
